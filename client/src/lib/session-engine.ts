...
export type TeacherSession = {
  ...
  transcriptionMeta?: {
    ...
    sources: Array<'browser-fallback' | 'mock-transcript' | 'whisper-failed'>;
    reason?: string;
  };
};
...
export type AppPreferences = {
  ...
  dashboardRange: 'week' | 'month';
  adminFilter: 'all' | 'excellent' | 'growing' | 'at-risk';
};
...
export function createSeedSessions(): TeacherSession[] {
  const seeds: SessionSeed[] = [
    { id: 'session-101', title: 'Multiplication with Stories', subject: 'Maths', className: 'Class 5A', schoolName: 'Govt. School No. 4', durationSeconds: 48, score: 82, status: 'completed', offline: false, language: 'hi' },
    { id: 'session-102', title: 'Weather Vocabulary Circle', subject: 'English', className: 'Class 4B', schoolName: 'Vidya Mandir', durationSeconds: 36, score: 74, status: 'completed', offline: false, language: 'en' },
    { id: 'session-103', title: 'Geometry Walkthrough', subject: 'Maths', className: 'Class 6A', schoolName: 'Z.P. School', durationSeconds: 41, score: 88, status: 'completed', offline: false, language: 'en' },
    { id: 'session-104', title: 'Science Observation Talk', subject: 'EVS', className: 'Class 3C', schoolName: 'Model High', durationSeconds: 29, score: 61, status: 'processing', offline: true, language: 'mr' },
    { id: 'session-105', title: 'Reading Fluency Check', subject: 'Language', className: 'Class 2A', schoolName: 'Satara North', durationSeconds: 24, score: 69, status: 'queued', offline: true, language: 'ta' },
  ];

  return seeds.map((seed) => createSessionFromSeed(seed));
}

export function buildTranscript(title: string, durationSeconds: number, language: SupportedLanguage): TranscriptLine[] {
...
export function buildReport(session: Pick<TeacherSession, 'subject' | 'className' | 'teacherName' | 'durationSeconds' | 'language' | 'transcript'>, baselineScore?: number): SessionReport {
  ...
  const teacherRatio = Math.round((teacherSpeakSeconds / durationSeconds) * 100);
  const studentRatio = Math.round((studentSpeakSeconds / durationSeconds) * 100);
  const silenceRatio = clamp(Math.round((silenceSeconds / durationSeconds) * 100), 0, 100);

  const talkRatio = {
    teacher: teacherRatio,
    student: studentRatio,
  ...
  const localized = getLocalizedReportCopy(session.language, { teacherLines, studentLines, waitTimeSeconds, finalScore, inclusionScore });
  ...
  return {
    ...
    coachingTips: createCoachingTips(session, finalScore, inclusionScore),
...
function getLocalizedReportCopy(language: SupportedLanguage, context: { teacherLines: number; studentLines: number; waitTimeSeconds: number; finalScore: number; inclusionScore: number; }) {
